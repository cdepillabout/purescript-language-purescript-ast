...
module Language.PureScript.AST.Exported where

import Prelude (bind, (&&), (==), (<>), (<<<), id, ($), map, (>>>))

import Data.Array (concatMap, (:), filter, mapMaybe)
import Data.Foldable (any, elem, all)
import Data.Maybe (Maybe(Just, Nothing))
import Data.Tuple (Tuple(Tuple), fst)

import Language.PureScript.AST.Declarations
    ( Declaration( PositionedDeclaration, TypeClassDeclaration, DataDeclaration
                 , TypeSynonymDeclaration, ExternDataDeclaration
                 , FixityDeclaration, ExternDeclaration, ValueDeclaration
                 , TypeDeclaration, TypeInstanceDeclaration
                 )
    , DeclarationRef( TypeRef, PositionedDeclarationRef, ProperRef
                    , TypeClassRef, ValueRef
                    )
    , Module(Module), flattenDecls
    )
import Language.PureScript.Types (Type(ConstrainedType, TypeConstructor), everythingOnTypes)
import Language.PureScript.Names (ProperName, Qualified(Qualified), runIdent)
...
exportedDeclarations :: Module -> Array Declaration
...
-- it unchanged.
filterDataConstructors :: Maybe (Array DeclarationRef) -> Declaration -> Declaration
...
-- produce incorrect results if this is not the case - for example, type class
-- instances will be incorrectly removed in some cases.
filterInstances :: Maybe (Array DeclarationRef) -> Array Declaration -> Array Declaration
...
typeInstanceConstituents :: Declaration -> Array (Qualified ProperName)
typeInstanceConstituents (TypeInstanceDeclaration _ constraints className tys _) =
  ...
  where
    fromConstraint :: Tuple (Qualified ProperName) (Array Type) -> Array (Qualified ProperName)
    fromConstraint (Tuple name tys') = name : concatMap fromType tys'

    fromType :: Type -> Array (Qualified ProperName)
    fromType = everythingOnTypes (<>) go
      where
        ...
        go :: Type -> Array (Qualified ProperName)
...
isExported :: Maybe (Array DeclarationRef) -> Declaration -> Boolean
isExported Nothing _ = true
isExported _ (TypeInstanceDeclaration _ _ _ _ _) = true
isExported exps (PositionedDeclaration _ _ d) = isExported exps d
isExported (Just exps) decl = any (matches decl) exps
  where
    matches :: Declaration -> DeclarationRef -> Boolean
...
isDctorExported :: ProperName -> Maybe (Array DeclarationRef) -> ProperName -> Boolean
isDctorExported _ Nothing _ = true
isDctorExported ident (Just exps) ctor = test `any` exps
  where
    test :: DeclarationRef -> Boolean
